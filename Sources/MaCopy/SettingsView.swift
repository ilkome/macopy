...
struct SettingsView: View {
    ...
    private static let faqEntries: [(q: String, a: String)] = [
        ...
        (
            "Как отредактировать скопированный текст?",
            "Выбери текстовый элемент и нажми ⌘E - курсор встанет в начало редактора в правой панели. Изменения сохраняются автоматически. Если оставить поле пустым и снять фокус, запись удаляется из истории."
        ),
        (
            "Как сохранить оригинал и редактировать копию?",
            "⌘D дублирует выбранный текст - копия появится в топе списка и сразу откроется в редакторе с курсором в начале. Оригинал остаётся нетронутым."
        ),
        (
            "Как открыть ссылку в браузере?",
            "Выбери URL-элемент и нажми ⌘⏎ - откроется в системном браузере, панель скроется. Для предпросмотра достаточно выделения - в правой панели показывается title, описание и картинка."
        ),
        (
            "Как добавить заметку к элементу?",
            "⌘W фокусирует поле комментария под превью. Комментарии ищутся наряду с основным содержимым - удобно для тегов вроде «client-X», «todo»."
        ),
        (
            "Как искать текст внутри скриншотов?",
            "Включи «OCR для скриншотов» выше. Распознанный текст индексируется вместе с обычным - можно искать по содержимому изображений из истории."
        ),
        (
